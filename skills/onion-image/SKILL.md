...
- 版位卡片必须展示 `target_size`、`render_size`、目标 KB、直出能力和不可选原因。双图/三图是可生成的图片形式；只有本次文案/用户意图已确定为单图时，双图/三图版位才临时置灰（反之亦然）。扩图、合成等当前不能直出的版位置灰，不允许提交。
...
- 界面 / 功能参考图不在 HTML 里上传；配置页只提供“需要界面/功能截图参考”开关。用户选择后，配置 JSON 必须写 `ui_reference_required=true`、`ui_reference_upload_status=awaiting_codex_upload`。
- 如果 `ui_reference_required=true`，读取配置后必须提醒用户回到 Codex 对话上传截图。没有收到截图前不能进入 prompt、validate-only 或 render；截图收到后再把它作为 `reference_images` 的一项，并在 prompt 参考图说明里标清“洋葱 APP 界面/功能截图”。
...
- 如果 `image-config-result.json` 里 `ui_reference_required=true`，但当前对话没有用户上传的界面/功能截图，下一步只能提醒用户上传截图；不能进入 prompt、validate-only 或 render。
...
用户给 C-XXX 但上下文没有文案内容时，先用 `../../shared/scripts/lookup_record.py --id C-XXX --follow-upstream` 回查。用户只给 D-XXX / 方向 ID 时，不查方向直接硬生图，也不启动 `/image-config`；明确转 `onion-copy` 先基于方向生成或确认文案。用户给 G-XXX、上传旧图或要求扩图/换图时，转 `onion-image-iterate`。用户只给临时文案时，不需要回查上游。
