...
用最小必要检查面回答"现在能不能跑、卡在哪里、Base 里还有什么"。它是诊断入口，不是工作流调度器；只解释现状和可选下一步，不替用户强行分流。首次安装或切设备时，可以执行 safe bootstrap：创建本地配置目录、复制 `.env.template`、创建本地输出目录、写 `setup-status.json`，但不静默填 API key、不代替用户完成飞书登录。
...
用户说"一键配置 / 初始化 / 第一次用 / 换电脑"时，优先运行：

```bash
python3 skills/onion-help/scripts/setup_wizard.py bootstrap
```

这个命令会自动判断 Mac / Windows / Linux，创建 `~/.onion-ad`（Windows 为用户目录下 `.onion-ad`）、复制 `.env.template`、创建由 `ONION_AD_OUTPUT_ROOT` 或系统临时目录决定的输出目录，并写入 `~/.onion-ad/setup-status.json`。输出里的 `platform.family` 用来判断后续提示应走 Mac 还是 Windows 口径。

bootstrap 只做安全本地动作。它不会安装 `lark-cli`、不会执行 `lark-cli auth login`、不会把真实 token 写进 `.env`。如果报告里出现缺 key、缺 lark-cli、未登录或权限不足，停下并给用户明确下一步。
