...
把已确认的文案转成可投放图片。这个 skill 会调用付费图片 API，因此必须先确认输入、生成 prompt、validate，再实际生图。
...
- `视觉配置`：版位、Logo、IP、CTA、界面/功能参考图、风格参考、套数。
...
- 未指定 Logo、CTA、IP 或界面/功能参考图时，不要替用户静默决定；在付费渲染前一次性确认。文本主导方案可以把“不用 IP”作为建议选项，但仍要问。
...
完整 brief 不弹配置卡。用户已经给了 C-XXX、IP、Logo、版位、形式、套数时，直接进入 prompt 和 validate。

非线性入口要承认：用户可能直接拿 C-XXX 生图，或拿一段临时文案生图；只要关键视觉配置足够，不要求先走方向和文案全流程。
...
有交互 UI 时一次性收集缺口；没有 UI 时用一句话列出关键缺口。具体运行时适配见 `../../shared/references/runtime-adapters.md`。

Codex / Claude Code 没有稳定原生选择卡时，优先使用本 skill 的本地交互页：启动 `scripts/interactive_server.py`，让用户在 `/image-config` 页面完成图片配置；保存后读取同目录的 `image-config-result.json`，再进入 prompt 和付费渲染。
...
缺视觉配置时使用 `scripts/interactive_server.py`：
...
脚本会输出 `url` 和 `result_path`。把 `url` 给用户打开，用户点击“保存配置”后读取 `<output-dir>/image-config-result.json`。这个 JSON 的 `placements[]` 是后续 prompt、`render.py --size`、导出 `target_size`、压缩 `target_kb`、参考图路径和 Base metadata 的唯一配置来源；不要再用聊天里的旧默认值覆盖它。若用户不能打开网页，再退回到文字选项，但仍按同一字段结构整理配置。
