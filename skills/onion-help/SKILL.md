...
用最小必要检查面回答"现在能不能跑、卡在哪里、Base 里还有什么"。它是诊断入口，不是工作流调度器；只解释现状和可选下一步，不替用户强行分流。首次安装或切设备时，可以执行 safe ensure/bootstrap：创建本地配置目录、复制 `.env.template`、创建本地输出目录、写 `setup-status.json` 和 `usage-state.json`，但不静默填 API key、不代替用户完成飞书登录。
...
这个命令会自动判断 Mac / Windows / Linux；首次使用或本地状态缺失时，会创建 `~/.onion-ad`（Windows 为用户目录下 `.onion-ad`）、复制 `.env.template`、创建由 `ONION_AD_OUTPUT_ROOT` 或系统临时目录决定的输出目录，并写入 `~/.onion-ad/setup-status.json` 与 `~/.onion-ad/usage-state.json`。输出里的 `first_use`、`auto_bootstrapped` 和 `platform.family` 用来判断后续提示应走首次配置、Mac 还是 Windows 口径。
