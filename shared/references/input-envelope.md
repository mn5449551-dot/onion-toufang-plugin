...
`creative_brief.level` 的 `full|lite|unknown` 三档语义统一见 `creative-brief.md`，不在本文件重复。底线只有一条：关键功能事实缺失且会影响方向、文案、图片 prompt、新卖点或换功能时，先查知识库、仍不清楚就追问；机械流程（配置、压缩、打包、写库、状态查询）不因 brief 不完整而阻塞。
