...
读取选择结果后，先用 `../onion-image/scripts/package_accepted_images.py` 打包 `accepted_schemes`，再用 `../../shared/scripts/write_image_group.py` 写入新 image_groups。基于 G-XXX 时传 `--parent-group-id`，有可信 directions/copies record_id 时传 `--direction-id` / `--copy-id`；临时上传图没有上游时可以不传。脚本默认上传压缩后的图片附件；如果版位规则给了明确 KB 上限，把目标值作为 `--target-kb` 或 metadata 的 `目标KB` 传入。写入成功后只更新实际关联到的原图组、文案和方向；已经是已用或废弃的记录不要重复改。
