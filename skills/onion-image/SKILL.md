...
把已确认的文案转成可投放图片。本 skill 调用付费图片 API；图片配置页是所有付费生图的强制入口（详见推断原则与硬约束）。
...
首启环境门禁：启动配置页、render、压缩打包或写 Base 前，确认 `~/.onion-ad/` 下 `usage-state.json`、`setup-status.json`、`update-status.json` 存在且无阻塞；缺失、过期或版本异常时转 `onion-help` 运行 `setup_wizard.py ensure`（24 小时缓存的更新检查与关闭开关等细节见 onion-help）。纯解释流程可继续；付费生图和外部写入不能跳过环境检查。

## 入口门禁

本 skill 只处理“基于已确认文案生成全新图片”。用户只给 D-XXX / 方向 ID 时，不进入本 skill 的生图流程：方向 ID 不是生图锚点，必须先转 `onion-copy` 形成或确认文案，不要启动配置页、不要直接用方向字段做图中文字。G-XXX、旧广告图、同款再来、扩同类、换文案、换角色、换 CTA 属于 `onion-image-iterate`；环境检查和 API key 问题转 `onion-help`。
...
- 不要靠是否有文案判断上传图角色，也不要靠图片像不像洋葱判断。用户上传作业帮等竞品/外部参考图时，除非明确说这是自有旧素材，否则只能当 `competitor_reference` / `layout_reference` / `style_reference` 使用，不写 `父图组`，prompt 只抽象参考构图、信息层级、节奏或色彩，不复刻竞品品牌、UI、人物或文案。用户说“参考这个作业帮版式给洋葱做一张”是新图参考素材；说“把这张作业帮图改成洋葱”也不当父图组迭代，应抽象成 layout_reference 重新创作。
- 上传图角色、图片用途、是否属于自有旧广告图有一点不清楚就追问（问法：这张图是自有旧广告图要改，还是竞品/构图/风格/IP 参考或 APP 截图？）；正确优先于少问。
...
- 图片套数默认 2 套（区别于 `onion-copy` 的文案套数）；用户说“几套”按形式换算张数，用户说“几张”时先澄清。
- 配置页里的“图片套数”含义固定为：每条文案在每个版位生成几套。必须展示并遵守 `预计图组数 = 文案数 × 版位数 × 套数`，最多 100 图组；双图/三图按 1 个图组计数，但内部会产生 2/3 个 render job。超过 100 时必须拆批，不自动截断。
...
批量变体默认由当前 Agent 分批生成 prompt。1-10 套可一次写完；11-50 套按 5-10 套一批推进，每批继承前面已用的 IP、场景、镜头、排版和视觉隐喻 ledger，保证跨批不重复。

屏幕内容闸门：界面截图不是独立入口。Agent 在写 prompt 前要判断最终画面是否会出现可识别的洋葱 APP/学习界面屏幕内容，例如拍题结果页、解析页、继续追问、步骤讲解、学习报告等。如果会出现，配置 JSON 必须有 `screen_ui_reference_required=true` 和兼容字段 `ui_reference_required=true`，并且必须先拿到用户在 Codex 对话上传的真实截图；没有截图时，只能改成弱化/模糊屏幕内容，或暂停等待用户上传。反向同样成立：画面不展示具体 UI、或屏幕只是弱化/模糊背景时，不需要也不要求用户上传截图。
...
- 缺付费生图所需资产选择时，只问缺失的视觉配置：Logo 要不要、CTA 写什么或不用、IP 用谁/随机/不用、是否使用用户上传图/风格参考图。界面/功能截图按屏幕内容闸门判断（见推断原则），不当常规缺口。IP 选项必须来自 `assets/asset-manifest.json` 的本地真实资产，不要只硬编码 6 个标准名；有全身照、学段、Q3 等变体时要列出可选变体。
...
- 版位先选大类：应用商店 / 学习机 / 信息流 / 百度；再在该大类下选具体版位。已选版位跨大类保留，允许同一文案一次生成多个渠道/版位。百度大类当前整体禁用（文案与素材口径未定），版位卡片置灰并展示原因；用户明确要求出百度素材时，直接说明百度暂不开放、口径确定后由维护人启用，不要尝试绕过配置页生成。
...
- 图片套数是手动数字输入，范围 1-50；含义是“每条文案在每个版位生成几套”。页面必须显示 `预计图组数 = 文案数 × 版位数 × 套数`，超过 100 图组时禁止保存并提示拆批；超过 10 套仍按批次推进 prompt 和渲染。
...
- Logo 按具体版位的 `logo_policy` 处理：绝大多数版位是 optional，用户自由选择；`forbidden` 的版位（如 OPPO 竞价 banner，厂商规范不允许加 Logo）不收 Logo、置灰并展示原因；`required` 的版位必须选择 Logo。用户打不开网页、退回文字选项收集配置时，同样必须遵守所选版位的 `logo_policy`。
- 界面 / 功能参考图不在 HTML 里上传；配置页只提供“画面需要展示洋葱 APP 界面/功能截图”开关。只有最终画面里出现手机、平板、电脑、学习机、投影屏、电子屏幕，并且屏幕要展示可识别的洋葱 APP/学习界面屏幕内容时才勾选。用户选择后，配置 JSON 必须写 `screen_ui_reference_required=true`、兼容字段 `ui_reference_required=true`、`ui_reference_upload_status=awaiting_codex_upload`；读取到后必须提醒用户回到 Codex 对话上传截图，没有收到截图前不能进入 prompt、validate-only 或 render，收到后把截图作为 `reference_images` 一项并在参考图说明里标清“洋葱 APP 界面/功能截图”。
- 补充说明只用于生图建议，例如画面风格、文字多少、主体位置，不承载结构化字段。
- 配置页默认是“探索生成”模式；“同类扩展 / 基于已有图继续扩”使用同一页面的 `generation_mode=iterate` 继承型配置。
...
- 成图后必须进入 `templates/image-selection.html` 选择页或等价的明确选择流程；不能只在聊天里贴图后结束，也不要让用户在聊天里回复“选 set1 / 选 set2 / 选第 N 套”来触发入库。只有选择页提交结果 `image-selection-result.json` 里 `accepted_schemes` 中的图组才能写入 Base；rejected、pending 不写库不打包；对话只能用于确认用户已提交，提交后 Agent 优先读取 `image-selection-result.json`，不能替代选择页标注，也不要让用户粘贴 JSON。
...
用户给 C-XXX 但上下文没有文案内容时，先用 `../../shared/scripts/lookup_record.py --id C-XXX --follow-upstream` 回查。用户只给 D-XXX / 方向 ID、G-XXX 或旧图时，按入口门禁转对应 skill；上传的 APP 截图、风格图、IP 参考图作为 `reference_images` 留在本 skill。用户只给临时文案时，不需要回查上游。
...
- `rejected_schemes` 里的标注反馈按 `../../shared/feedback_observation.md` 的字段映射写入 `feedbacks`（固定规则反馈 / 主观感受反馈两类可同写，`skip_feedback=true` 不写）。被反馈对象 ID 优先用已有 G-ID；未采纳方案用 `<request_id>/<set_id>` 临时锚点并补齐 `关联文案` 等追溯字段（见 `../../shared/base_schema.md` 批量出图追溯规则）。
...
写入 Base 前先处理选择页反馈、再打包采纳图（顺序与状态机一致：反馈 → 打包 → 写库）：
...
随后用 `scripts/package_accepted_images.py` 打包采纳图（命令与参数见 `references/压缩与导出.md`）。脚本输出的 zip 是给运营本地留档/交付的“通过图片包”。zip 里只能放图片文件，不放 manifest、HTML、JSON 或未采纳方案。用户只填方向名，不填文件夹名或图片名；脚本按配置页的版位大类、渠道平台和版位自动生成，例如 `方向31.zip`、`方向31-应用商店-华为-大卡智投/方向31-应用商店-华为-大卡智投-1.jpg`。如果同一大类/渠道/版位下存在不同目标尺寸，脚本会在文件夹和图片名前追加尺寸以避免冲突。Base 追溯仍靠 request_id、copy_id、set_id、manifest 和 image-write-result，不靠这个交付包名称。
...
原图清理默认自动发生在 `write_image_group.py` 成功写 Base 后；手动预检命令见 `references/压缩与导出.md`。
