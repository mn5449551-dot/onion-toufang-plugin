...
写 Base 时必须传 `--write-result /tmp/onion-ad/<request_id>/image-write-result.json`，或确保图片路径能让脚本自动把完成标记写到同一输出目录。完成标记存在后，`image_workflow.py status` 会返回 `complete`，避免同一 request 重复写入 image_groups。
