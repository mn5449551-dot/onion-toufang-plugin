...
- `改动轴`：IP、文案、场景细节、版位、Logo/CTA、风格参考图等。用户说“换比例”时按换版位理解，比例由版位规则绑定。
...
- 原图组的渠道、图片形式、版位、关联文案、关联方向、Logo、IP、原始 prompt 和附件参考图；用户给 G-XXX 但上下文没有内容时先回查 Base。
...
扩同类默认继承原图组的 Creative Brief / metadata：原功能、目标用户、核心卖点、产品动作和可感知变化尽量不漂移。换卖点、换功能、换文案时重新建立 Lite/Full Brief；纯视觉微调不强制补全功能事实。
...
- 缺力度时，问微调 / 扩同类 / 换形式。
- 缺改动轴时，问具体想改 IP、文案、场景、版位、Logo/CTA、风格中的哪些。
...
- 批量渲染必须走 `../onion-image/scripts/batch_render.py`，不要手工并发多个 render.py。并发单位是 render job，不是套数；KIE 默认并发、降级重试与任务恢复规则见 `../../shared/recipes/render-chain.md`，不做团队级全局锁。
...
- 创意理解继承和功能事实门禁：`../../shared/references/creative-brief.md`。
...
单 job 渲染仍使用 `../onion-image/scripts/render.py`，按 `../../shared/recipes/render-chain.md` 的单张接口和链式规则执行。批量多套迭代时先生成 `image-render-manifest.json`，再调用 `../onion-image/scripts/batch_render.py`；它会复用 `render.py`、跳过已存在 PNG、按依赖保护双/三图链路，并输出 `image-render-result.json`。
